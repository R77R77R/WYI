...
DECLARE cursor_Market_Instrument CURSOR FOR 
    SELECT name FROM SYSCOLUMNS WHERE id=object_id('Market_Instrument') AND (name NOT IN ('ID','Createdat','Updatedat','Sort','DescTxt','Code','Caption','Long','LongCode','Short','ShortCode','PriceCurrent'))
...
-- [Market_Instrument.PriceCurrent] -------------


-- [Market_Instrument.PriceCurrent] -------------

IF EXISTS(SELECT * FROM SYSCOLUMNS WHERE id=object_id('Market_Instrument') AND name='PriceCurrent')
    BEGIN
     ALTER TABLE Market_Instrument ALTER COLUMN [PriceCurrent] FLOAT
    END
ELSE
    BEGIN
    DECLARE @sql_add_Market_Instrument_PriceCurrent NVARCHAR(MAX);
    SET @sql_add_Market_Instrument_PriceCurrent = 'ALTER TABLE Market_Instrument ADD [PriceCurrent] FLOAT'
    EXEC sp_executesql @sql_add_Market_Instrument_PriceCurrent
    END


IF EXISTS(SELECT object_id FROM [sys].[objects] WHERE name='Constraint_Market_InstrumentPriceCurrent')
    BEGIN
    ALTER TABLE Market_Instrument DROP  CONSTRAINT [Constraint_Market_InstrumentPriceCurrent]
    END

IF EXISTS(SELECT * FROM SYSINDEXES WHERE name='UniqueNonclustered_Market_InstrumentPriceCurrent')
    BEGIN
    ALTER TABLE Market_Instrument DROP  CONSTRAINT [UniqueNonclustered_Market_InstrumentPriceCurrent]
    END
-- [Market_Ticket] ----------------------
