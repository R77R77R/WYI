...
const wsbinHandler = async (msg) => {
  ({ bin: await msg.arrayBuffer(), index: 0 });
};
class BytesBuilder {
  count;
  buffer;
  ...
  append(bin2) {
    this.buffer.push(bin2);
    this.count += bin2.byteLength;
  }
  bytes() {
    let res = new ArrayBuffer(this.count);
    let i = 0;
    this.buffer.forEach((bin2) => {
      new Uint8Array(res).set(new Uint8Array(bin2), i);
      i += bin2.byteLength;
    });
    return res;
  }
...
const Mors__BytesBuilder = (fn) => (data) => {
  const bb = new BytesBuilder();
  fn(bb)(data);
  return bb;
};
const ArrayBuffer__base64 = (buffer2) => {
...
const data__base64 = (data__bin) => (data) => {
  return Mors__BytesBuilder(data__bin)(data).base64();
};
const ArrayBuffer__hex = (buffer2) => {
  const uint8Array = new Uint8Array(buffer2);
  let hexString = "";
  for (let i = 0; i < uint8Array.length; i++) {
    const hex2 = uint8Array[i].toString(16).padStart(2, "0");
    hexString += hex2 + " ";
  }
  const hex = hexString.trim();
  return hex;
};
const string__ArrayBuffer = (str) => {
  const stringLength = str.length;
  const buffer2 = new ArrayBuffer(stringLength * 2);
  const bufferView = new Uint16Array(buffer2);
  for (let i = 0; i < stringLength; i++) {
    bufferView[i] = str.charCodeAt(i);
  }
  return buffer2;
};
const int32__binL = (bb) => (v) => {
  const bin2 = new Int32Array([v]).buffer;
  bb.append(bin2);
};
const int32__bin = (bb) => (v) => {
  const buffer2 = new ArrayBuffer(4);
  const view = new DataView(buffer2);
  view.setInt32(0, v, true);
  bb.append(buffer2);
};
const bin__int32 = (bi, littleEndian = true) => {
  const v = new DataView(bi.bin);
  const res = v.getInt32(bi.index, littleEndian);
  bi.index += 4;
  return res;
};
const int64__bin = (bb) => (v) => {
  if (typeof v == "number") {
    v = BigInt(v);
  }
...
const bin__int64 = (bi, littleEndian = true) => {
  const v = new DataView(bi.bin);
  const res = v.getBigInt64(bi.index, littleEndian);
  bi.index += 8;
  return Number(res);
};
const DateTime__bin = (bb) => (v) => {
  const checkDate = (v2) => {
    if (typeof v2 == "number") return v2;
      ...
      try {
        ...
        console.log(`something wrong with ${v2}, return current Date.`);
...
const bin__DateTime = (bi, littleEndian = true) => {
  const view = new DataView(bi.bin);
  const timestamp = view.getBigInt64(bi.index, littleEndian);
  const res = new Date(Number(timestamp));
  bi.index += 8;
  return res;
};
const float__bin = (bb) => (v) => {
  const bin2 = new Float64Array([v]).buffer;
  bb.append(bin2);
};
const bin__float = (bi) => {
  const v = new DataView(bi.bin);
...
const bool__bin = (bb) => (v) => {
  const bin2 = new Uint8Array([v ? 1 : 0]).buffer;
  bb.append(bin2);
};
const bin__bool = (bi) => {
  const v = new DataView(bi.bin);
...
const Json__bin = (bb) => (obj) => {
  str__bin(bb)(JSON.stringify(obj));
};
const option__bin = (item__bin) => (bb) => (v) => {
  if (v) {
    bool__bin(bb)(true);
    item__bin(bb)(v);
  } else
...
const bin__option = (bin__item) => (bi) => {
  if (bin__bool(bi))
    return bin__item(bi);
  else
    return null;
};
const array__bin = (item__bin) => (bb) => (array) => {
  int32__bin(bb)(array.length);
  array.forEach((i) => {
    item__bin(bb)(i);
  });
};
const bin__array = (bin__item) => (bi) => {
  let res = [];
...
const bin__int32array = bin__array(bin__int32);
const bin__int64array = bin__array(bin__int64);
const bin__boolarray = bin__array(bin__bool);
const List__bin = (item__bin) => (bb) => (array) => {
  int32__bin(bb)(array.length);
  array.forEach((i) => {
    item__bin(bb)(i);
  });
};
const bin__List = (bin__item) => (bi) => {
  let res = [];
  if (bi.bin.byteLength >= 4) {
    let length = bin__int32(bi);
    for (let i = 0; i < length; i++) {
...
const dict__kvpArray = (dict) => {
  return Object.entries(dict).map(([k, v]) => ({ k, v }));
};
const dict__bin = (k__bin) => (v__bin) => (bb) => (dict) => {
...
const bin__dict = (bin__k) => (bin__v) => (bi) => {
  const dict = {};
  if (bi.bin.byteLength >= 4) {
    let length = bin__int32(bi);
    for (let i = 0; i < length; i++) {
...
const createWebSocket_base = (wsbinHandler2, wsjsonHandler2) => (wsurl) => (savetoRT = "ws.conn") => {
  ...
  glib.setRT(savetoRT, ws2);
  return ws2;
};
const createWebSocket = createWebSocket_base(wsbinHandler);
const disconnect = (ws2 = rtxx.wsctx) => {
  if (ws2.ws && ws2.ws.readyState === WebSocket.OPEN) {
    ws2.ws.close();
  }
};
const trySend = (x) => (e) => (msg) => {
  if (!x) {
    x = rtxx.wsctx;
  }
  if (x && x.ws.readyState === WebSocket.OPEN) {
    const bb = new BytesBuilder();
    glib.Mor.Msg__bin(bb)({ e: 1, val: msg });
    x.ws.send(bb.bytes());
  }
};
const trySendx = (e) => (msg) => {
  trySend(rtxx.wsctx)(e)(msg);
};
const tryApiRequest = async (req, rep) => {
...
const parse_md = (str) => {
  ...
  if (tag) {
    for (let i = 0, len = tag.length; i < len; i++) {
      ...
      const lang2 = tagPart.length === 2 ? tagPart[0] : "zh";
      str = str.replace(tag[i], `<a href="/community/${lang2}:${word}" title="${word}" class="tag">${word}</a>`);
    }
  }
  ...
  for (let i = 0, len = p2.length; i < len; i++) {
    ...
    if (matchArr) {
      ...
      switch (matchArr[0]) {
        ...
        case "```":
          i++;
          while (i < len && !re4.test(p2[i])) {
            temp += escape_html(p2[i]);
            temp += "\n";
            i++;
          }
...
const markdown__html_ = (str) => {
  if (!str) str = "";
  try {
    const p2 = str.split(/\r?\n/);
    return compile_md(p2);
  } catch {
    compile_md([str]);
  }
  return "";
};
const html__toc = (html) => {
  const parser = new DOMParser();
  const doc2 = parser.parseFromString(html, "text/html");
  const headings = doc2.querySelectorAll("h1, h2, h3, h4, h5, h6");
  const counter = [0];
  let tocHtml = "";
  headings.forEach((heading) => {
    ...
    const numbering = counter.slice(0, level).join(".");
    const listItem = `<li><a href="#${id}">${numbering} ${text}</a></li>`;
    tocHtml += listItem;
  });
  return tocHtml;
};
...
const Vct2_xOffset = (v) => (x) => {
  return {
    x: v.x + x,
    y: v.y
  };
};
const Vct2_yOffset = (v) => (y) => {
  return {
    x: v.x,
    y: v.y + y
  };
...
const checkPRange = (coord) => (p2) => {
  if (coord.pinf > p2)
    coord.pinf = p2;
  if (coord.psup < p2)
    coord.psup = p2;
};
const checkmultiPRange = (coord) => (ps) => {
  ps.forEach(checkPRange(coord));
};
const strechPRange = (coord) => (rate) => {
  let inf = coord.pinf;
  let sup = coord.psup;
...
const pxy__dxy = (coordxy) => (v) => {
  return {
    x: p__d(coordxy.x)(v.x),
    y: p__d(coordxy.y)(v.y)
  };
};
const Chart__CoordXY = (chart) => {
  return {
    x: {
      dinf: chart.l,
...
const colrowSpan__gridLayoutCell = (col, row, colspan, rowspan) => {
  return {
    col,
    row,
    colspan,
...
const colrow__gridLayoutCell = (col, row) => {
  return {
    col,
    row,
    colspan: 1,
...
const percentage__controlPoints = (inf, sup) => (vs) => {
  let sum = 0;
  vs.forEach((v) => {
    sum += v;
  });
...
const chart__layout = (chart) => (xs, ys) => (grids) => {
...
const graphics = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  Chart__CoordXY,
  RowCol__Cells,
  Vct2_xOffset,
...
const biz__LoginOptions = (biz = "DISCORD") => {
  const options = {
    biz: biz.toUpperCase(),
    code: "",
...
const LoginOption__RT = async (options) => {
...
const authParams__URLQuery = (params) => {
  return Object.keys(params).map((key) => encodeURIComponent(key) + "=" + encodeURIComponent(params[key])).join("&");
};
const host__DiscordRedirectURL = (host2 = rtxx.host) => {
  const p2 = {
    client_id: host2.discordAPPID,
    response_type: "code",
    redirect_uri: host2.discordRedirect,
    scope: "identify"
  };
  const params = authParams__URLQuery(p2);
...
const auth = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  SignOut,
  biz__LoginOptions,
  host__DiscordRedirectURL
}, Symbol.toStringTag, { value: "Module" }));
const add = (msg) => {
  msg_add({ msg });
};
const aSuc = (msg, exp = 400) => {
  msg_add({ msg, expire: exp, bgcolor: "bg-green-500/80" });
};
const aFail = (msg, exp = 1500) => {
  msg_add({ msg, expire: exp, bgcolor: "bg-red-500/80" });
};
const aEx = (msg, exp = 1500) => {
  msg_add({ msg, expire: exp, bgcolor: "bg-orange-500/80" });
};
const msg_add = (item) => {
...
const notify = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  DefaultItem,
  Refresh,
  aEx,
...
const loader = async (url, post2, h2) => {
  post2.session = runtime.session;
  let rep = await glib.post(url, post2);
  if (rep?.Er == "OK") {
    h2(rep);
  }
};
const _hoisted_1$3 = { class: "main-color lg:w-screen text-2xl" };
const _hoisted_2$3 = /* @__PURE__ */ createBaseVNode("div", { class: "hor" }, null, -1);
const _hoisted_3$3 = {
  id: "home",
...
const _hoisted_29 = /* @__PURE__ */ createBaseVNode("div", { class: "flex justify-center" }, [
  /* @__PURE__ */ createBaseVNode("div", { class: "hor-range" }, [
    /* @__PURE__ */ createBaseVNode("div", null, "info@sdchen.art"),
...
const _hoisted_1$2 = { class: "flex justify-center" };
const _hoisted_2$2 = { class: "hor-range" };
const _hoisted_3$2 = /* @__PURE__ */ createBaseVNode("div", { class: "card" }, [
  /* @__PURE__ */ createBaseVNode("div", { class: "card-caption" }, "Admin Console")
], -1);
const _hoisted_4$2 = { class: "card" };
const _hoisted_5$2 = /* @__PURE__ */ createBaseVNode("div", { class: "card-caption" }, "Book", -1);
        ...
        createBaseVNode("div", _hoisted_1$2, [
          createBaseVNode("div", _hoisted_2$2, [
            ...
            createBaseVNode("div", _hoisted_4$2, [
              _hoisted_5$2,
              (openBlock(true), createElementBlock(Fragment, null, renderList(unref(s).books, (book) => {
...
const routes = [
  ...
  { path: "/admin", component: _sfc_main$2 }
];
const router = createRouter({
  history: createMemoryHistory(),
  routes
});
const navigate = (href, name, id) => {
  let url = href;
  if (url.indexOf("/zh/") > 0) {
    runtime.lang = "zh";
...
const initHost = () => {
  ...
  switch (host2.hostname) {
    case "localhost":
      host2.hostname = "localhost";
      host2.api = "http://localhost:2024";
      host2.wsurl = "wss://localhost/";
      break;
    case "127.0.0.1":
      host2.hostname = "127.0.0.1";
      host2.api = "http://localhost";
      host2.wsurl = "wss://localhost/";
      break;
  }
  return host2;
};
const host$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  checkDomain,
  initHost
}, Symbol.toStringTag, { value: "Module" }));
...
const pFILE__bin = (bb) => (p2) => {
  marshall$1.str__bin(bb)(p2.Caption);
  marshall$1.str__bin(bb)(p2.Desc);
  marshall$1.str__bin(bb)(p2.Suffix);
  marshall$1.int64__bin(bb)(p2.Size);
  marshall$1.int32__bin(bb)(p2.Thumbnail.length);
  bb.append(p2.Thumbnail);
  marshall$1.int64__bin(bb)(p2.Owner);
};
const FILE__bin = (bb) => (v) => {
  marshall$1.int64__bin(bb)(v.id);
  marshall$1.int64__bin(bb)(v.sort);
  marshall$1.DateTime__bin(bb)(v.createdat);
  marshall$1.DateTime__bin(bb)(v.updatedat);
...
const bin__pFILE = (bi) => {
  let p2 = pFILE_empty();
  p2.Caption = marshall$1.bin__str(bi);
  p2.Desc = marshall$1.bin__str(bi);
  p2.Suffix = marshall$1.bin__str(bi);
  p2.Size = marshall$1.bin__int64(bi);
  let lengthThumbnail = bin__int32(bi);
  p2.Thumbnail = bi.bin.slice(bi.index, lengthThumbnail);
  bi.index += lengthThumbnail;
  p2.Owner = marshall$1.bin__int64(bi);
  return p2;
};
const bin__FILE = (bi) => {
  let ID = marshall$1.bin__int64(bi);
...
const pFBIND__bin = (bb) => (p2) => {
  marshall$1.int64__bin(bb)(p2.File);
  marshall$1.int64__bin(bb)(p2.Moment);
  marshall$1.str__bin(bb)(p2.Desc);
};
const FBIND__bin = (bb) => (v) => {
  marshall$1.int64__bin(bb)(v.id);
  marshall$1.int64__bin(bb)(v.sort);
  marshall$1.DateTime__bin(bb)(v.createdat);
  marshall$1.DateTime__bin(bb)(v.updatedat);
...
const bin__pFBIND = (bi) => {
  let p2 = pFBIND_empty();
  p2.File = marshall$1.bin__int64(bi);
  p2.Moment = marshall$1.bin__int64(bi);
  p2.Desc = marshall$1.bin__str(bi);
  return p2;
};
const bin__FBIND = (bi) => {
...
const pMOMENT__bin = (bb) => (p2) => {
  marshall$1.str__bin(bb)(p2.Title);
  marshall$1.str__bin(bb)(p2.Summary);
  marshall$1.str__bin(bb)(p2.FullText);
  marshall$1.str__bin(bb)(p2.PreviewImgUrl);
  marshall$1.str__bin(bb)(p2.Link);
  marshall$1.int32__bin(bb)(p2.Type);
  marshall$1.int32__bin(bb)(p2.State);
  marshall$1.int32__bin(bb)(p2.MediaType);
};
const MOMENT__bin = (bb) => (v) => {
  marshall$1.int64__bin(bb)(v.id);
  marshall$1.int64__bin(bb)(v.sort);
  marshall$1.DateTime__bin(bb)(v.createdat);
  marshall$1.DateTime__bin(bb)(v.updatedat);
...
const bin__pMOMENT = (bi) => {
  let p2 = pMOMENT_empty();
  p2.Title = marshall$1.bin__str(bi);
  p2.Summary = marshall$1.bin__str(bi);
  p2.FullText = marshall$1.bin__str(bi);
  p2.PreviewImgUrl = marshall$1.bin__str(bi);
  p2.Link = marshall$1.bin__str(bi);
  p2.Type = marshall$1.bin__int32(bi);
  p2.State = marshall$1.bin__int32(bi);
  p2.MediaType = marshall$1.bin__int32(bi);
  return p2;
};
const bin__MOMENT = (bi) => {
  let ID = marshall$1.bin__int64(bi);
  let Sort = marshall$1.bin__int64(bi);
  let Createdat = marshall$1.bin__DateTime(bi);
  let Updatedat = marshall$1.bin__DateTime(bi);
...
const pLOG__bin = (bb) => (p2) => {
  marshall$1.str__bin(bb)(p2.Location);
  marshall$1.str__bin(bb)(p2.Content);
  marshall$1.str__bin(bb)(p2.Sql);
};
const LOG__bin = (bb) => (v) => {
  marshall$1.int64__bin(bb)(v.id);
  marshall$1.int64__bin(bb)(v.sort);
...
const bin__pLOG = (bi) => {
  let p2 = pLOG_empty();
  p2.Location = marshall$1.bin__str(bi);
  p2.Content = marshall$1.bin__str(bi);
  p2.Sql = marshall$1.bin__str(bi);
  return p2;
};
const bin__LOG = (bi) => {
  let ID = marshall$1.bin__int64(bi);
...
const pPLOG__bin = (bb) => (p2) => {
  marshall$1.str__bin(bb)(p2.Ip);
  marshall$1.str__bin(bb)(p2.Request);
};
const PLOG__bin = (bb) => (v) => {
  marshall$1.int64__bin(bb)(v.id);
  marshall$1.int64__bin(bb)(v.sort);
  marshall$1.DateTime__bin(bb)(v.createdat);
  marshall$1.DateTime__bin(bb)(v.updatedat);
...
const bin__pPLOG = (bi) => {
  let p2 = pPLOG_empty();
  p2.Ip = marshall$1.bin__str(bi);
  p2.Request = marshall$1.bin__str(bi);
  return p2;
};
const bin__PLOG = (bi) => {
  let ID = marshall$1.bin__int64(bi);
...
const momentTypeEnum_Original = 0;
const momentTypeEnum_Repost = 1;
const momentTypeEnum_Thread = 2;
const momentTypeEnum_Forum = 3;
const momentTypeEnum_Question = 4;
const momentTypeEnum_Answer = 5;
const momentTypeEnum_BookmarkList = 6;
const momentTypeEnum_Poll = 7;
const momentTypeEnum_Miles = 8;
const momentTypeEnum_Dict = 9;
const momentTypeEnum_WebPage = 10;
const momentTypeEnum_MediaFile = 11;
const momentStateEnum_Normal = 0;
const momentStateEnum_Deleted = 1;
const momentStateEnum_Scratch = 2;
const momentMediaTypeEnum_None = 0;
const momentMediaTypeEnum_Video = 1;
const momentMediaTypeEnum_Audio = 2;
const pMOMENT_empty = () => {
  return {
    Title: "",
    Summary: "",
...
const om = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  bin__pMOMENT,
  bin__pPLOG,
  euAuthTypeEnum_Admin,
  euAuthTypeEnum_Authorized,
  euAuthTypeEnum_Normal,
  momentMediaTypeEnum_Audio,
  momentMediaTypeEnum_None,
  momentMediaTypeEnum_Video,
  momentStateEnum_Deleted,
  momentStateEnum_Normal,
  momentStateEnum_Scratch,
  momentTypeEnum_Answer,
  momentTypeEnum_BookmarkList,
  momentTypeEnum_Dict,
  momentTypeEnum_Forum,
  momentTypeEnum_MediaFile,
  momentTypeEnum_Miles,
  momentTypeEnum_Original,
  momentTypeEnum_Poll,
  momentTypeEnum_Question,
...
const marshall = { ...bin, ...om };
var MsgEnum = /* @__PURE__ */ ((MsgEnum2) => {
  MsgEnum2[MsgEnum2["Heartbeat"] = 0] = "Heartbeat";
  MsgEnum2[MsgEnum2["ApiRequest"] = 1] = "ApiRequest";
  MsgEnum2[MsgEnum2["ApiResponse"] = 2] = "ApiResponse";
  return MsgEnum2;
})(MsgEnum || {});
var ErEnum = /* @__PURE__ */ ((ErEnum2) => {
  ErEnum2[ErEnum2["ApiNotExists"] = 0] = "ApiNotExists";
  ErEnum2[ErEnum2["InvalideParameter"] = 1] = "InvalideParameter";
  ErEnum2[ErEnum2["Unauthorized"] = 2] = "Unauthorized";
  ErEnum2[ErEnum2["NotAvailable"] = 3] = "NotAvailable";
  ErEnum2[ErEnum2["Internal"] = 4] = "Internal";
  return ErEnum2;
})(ErEnum || {});
const EuComplex_empty = () => {
  return {
    eu: { id: 0, sort: 0, createdat: /* @__PURE__ */ new Date(), updatedat: /* @__PURE__ */ new Date(), p: marshall.pEU_empty() }
  };
};
const EuComplex__bin = (bb) => (v) => {
  marshall.EU__bin(bb)(v.eu);
};
const bin__EuComplex = (bi) => {
  return {
    eu: marshall.bin__EU(bi)
  };
};
const MomentComplex_empty = () => {
  return {
    m: { id: 0, sort: 0, createdat: /* @__PURE__ */ new Date(), updatedat: /* @__PURE__ */ new Date(), p: marshall.pMOMENT_empty() }
  };
};
const MomentComplex__bin = (bb) => (v) => {
  marshall.MOMENT__bin(bb)(v.m);
};
const bin__MomentComplex = (bi) => {
  return {
...
const ClientRuntime__bin = (bb) => (v) => {
  marshall.int32__bin(bb)(v.version);
};
const bin__ClientRuntime = (bi) => {
  return {
...
let wsOnOpen = (runtime2) => (event) => {
  console.log("WebSocket connected");
};
let wsOnClose = (runtime2) => (event) => {
  console.log("WebSocket closed");
};
let wsOnError = (runtime2) => (event) => {
  console.log(event);
};
let wsOnMsg = (runtime2) => (event) => {
  let msg = event.data;
  switch (msg.e) {
...
const initRuntime = (runtime2) => {
  runtime2.host = initHost();
  let ws2 = new WebSocket(runtime2.host.wsurl);
  ws2.onopen = wsOnOpen();
  ws2.onclose = wsOnClose();
  ws2.onerror = wsOnError();
  ws2.onmessage = wsOnMsg();
  runtime2.wsctx.ws = ws2;
};
const loadLS = (key, defaultv = "{}") => {
...
const prepRuntime = () => {
  ...
  initRuntime(runtime2);
  return runtime2;
};
const getRT = (key, obj = runtime) => {
  const keys = key.split(".");
  return keys.reduce((acc, curr) => acc?.[curr], obj);
};
const setNested = (key, value, f_map = (value2) => {
  return value2;
}, obj = runtime) => {
  const keys = key.split(".");
...
const setRT = setNested;
const createGlobalWatcher = () => {
  const createwatch = (rtkeys2) => {
    for (const rtkey of rtkeys2) {
    }
  };
  const rtkeys = [
    "session",
    ...
    return (_ctx, _cache) => {
      ...
      return openBlock(), createElementBlock("div", null, [
        createBaseVNode("div", _hoisted_1$1, [
          ...
          createBaseVNode("button", {
            onClick: _cache[1] || (_cache[1] = ($event) => unref(s).expand = !unref(s).expand)
          }, "Toggle")
        ]),
        unref(s).expand ? (openBlock(), createElementBlock("div", _hoisted_6$1, [
          unref(s).user.eu.id > 0 ? (openBlock(), createElementBlock("div", _hoisted_7$1, [
            unref(s).user.eu.p.AuthType == 2 ? (openBlock(), createElementBlock("div", _hoisted_8$1, [
            ...
            createBaseVNode("div", null, [
              createBaseVNode("div", _hoisted_10, [
                ...
                withDirectives(createBaseVNode("input", {
                  ...
                  [vModelText, unref(s).key]
                ])
              ])
            ]),
            createBaseVNode("div", null, [
              createBaseVNode("button", { onClick: authKey }, "Authorize")
            ])
          ])
        ])) : createCommentVNode("", true)
      ]);
...
const _hoisted_13 = /* @__PURE__ */ createBaseVNode("div", { class: "flex justify-center" }, [
  /* @__PURE__ */ createBaseVNode("div", { class: "hor-range" }, [
    /* @__PURE__ */ createBaseVNode("div", { class: "pt-2 pb-5" }, [
      /* @__PURE__ */ createBaseVNode("br"),
      /* @__PURE__ */ createTextVNode("© 2025 CiteLand ")
    ])
  ])
], -1);
const _sfc_main = {
...
if (localUser)
  runtime.user = JSON.parse(localUser);
else
  runtime.user = glib.Mor.studio.EuComplex_empty();
glib.runtime.createGlobalWatcher();
glib.notify.init();
const app = glib.vue.createApp(_sfc_main);
